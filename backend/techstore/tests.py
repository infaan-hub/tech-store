...
from techstore_backend import settings as project_settings
...
class ProductApiTests(APITestCase):
    ...
    def test_public_products_list_returns_503_when_database_fails(self):
        with patch("techstore.views.ProductViewSet.get_queryset", side_effect=DatabaseError("db down")):
            response = self.client.get("/api/products/")
    ...
    def test_public_product_detail_returns_503_when_database_fails(self):
        ...
        with patch("techstore.views.ProductViewSet.get_object", side_effect=DatabaseError("db down")):
    ...
    def test_product_create_returns_503_when_storage_fails(self):
        ...
        with patch("techstore.views.ProductViewSet.perform_create", side_effect=PermissionError("disk denied")):
            response = self.client.post(
                "/api/products/",
                {
                    "name": "Fresh Lemon",
...
class CustomerOrderApiTests(APITestCase):
    ...
    def test_checkout_returns_503_when_database_write_fails(self):
        ...
        with patch("techstore.views.Sale.objects.create", side_effect=DatabaseError("db down")):
            response = self.client.post(
                "/api/customer/checkout/",
                {
    ...
    def test_checkout_succeeds_when_barcode_image_generation_fails(self):
        ...
        with patch("techstore.serializers.createBarcodeDrawing", return_value=broken_drawing):
            response = self.client.post(
                "/api/customer/checkout/",
                {
                    "items": [{"product": self.product.id, "quantity": 1}],
